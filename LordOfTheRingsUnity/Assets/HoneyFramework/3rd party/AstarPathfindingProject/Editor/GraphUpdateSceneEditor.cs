...
namespace Pathfinding {
	...
	public class GraphUpdateSceneEditor : Editor {
		...
		public void OnSceneGUI () {
			...
			if (Tools.current != Tool.View && Event.current.type == UnityEngine.EventType.Layout) {
			...
			if (Event.current.shift && Tools.current == Tool.Move) {
				...
				if (((int)Event.current.modifiers & (int)EventModifiers.Alt) != 0) {
					if (Event.current.type == UnityEngine.EventType.MouseDown && selectedPoint >= 0 && selectedPoint < points.Count) {
